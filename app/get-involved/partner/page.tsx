            ...
            <div className="lg:col-span-5">
              ...
              <div className="mt-8 flex flex-col sm:flex-row gap-4 sm:items-center">
                ...
                <a
                  ...
                  className="inline-flex items-center justify-center gap-2 px-8 py-4 rounded-full border-2 border-brand-green text-brand-dark font-medium hover:bg-brand-green/10 transition-colors w-full sm:w-auto"
                >
                  Download brochure
                </a>
              </div>
              ...
              <div className="grid sm:grid-cols-2 gap-5">
                {proofPoints.map((p) => (
                  ...
                  >
                    ...
                    <h3 className="mt-4 font-heading text-xl font-semibold text-brand-dark">{p.title}</h3>
                    <p className="mt-2 text-brand-dark/75 leading-relaxed">{p.desc}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="container-wide">
          ...
          <div className="grid md:grid-cols-2 gap-6 sm:gap-8">
            {pathways.map((card) => (
              <div
                key={card.title}
                className="rounded-[2.5rem] border border-slate-200 bg-white p-6 sm:p-10 shadow-sm hover:shadow-lg hover:shadow-slate-200/40 transition-shadow"
              >
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="font-heading text-2xl font-semibold text-brand-dark">{card.title}</h3>
                    <p className="mt-2 text-brand-dark/75 leading-relaxed">{card.desc}</p>
                  </div>
                </div>
        ...
        <div className="container-wide">
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            <div className="rounded-[2.5rem] border border-slate-200 bg-slate-50 p-6 sm:p-10">
              <h3 className="font-heading text-lg font-semibold text-brand-dark mb-6">Current partners</h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-6 items-center">
                {currentPartners.map((logo) => (
                  <div
                    key={logo.src}
                    className="flex items-center justify-center rounded-2xl bg-white border border-slate-200 p-5 h-24 hover:border-brand-green/30 hover:shadow-sm transition-all"
                  >
                    <Image
                      src={logo.src}
                      alt={logo.alt}
            ...
            <div className="rounded-[2.5rem] border border-slate-200 bg-slate-50 p-6 sm:p-10">
              <h3 className="font-heading text-lg font-semibold text-brand-dark mb-6">Past donors</h3>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-6 items-center">
                {pastDonors.map((logo) => (
                  <div
                    key={logo.src}
        ...
        <div className="max-w-7xl mx-auto">
          <div className="rounded-[2.5rem] border border-white/10 bg-white/5 backdrop-blur-sm p-6 sm:p-10 flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto">
              <Link
                href="/contact"
                className="inline-flex items-center justify-center px-8 py-4 rounded-full bg-white text-brand-dark font-medium hover:bg-brand-sunlight transition-colors w-full sm:w-auto"
              >
                Contact us
              </Link>
              <Link
