...
export default async function CareersPage() {
  const [content, careers] = await Promise.all([getWebsiteContent(), getCareers()]);
  ...
  return (
    <>
      <Hero
        ...
        primaryCta={t("careers.hero.primary_cta")}
        primaryHref="/contact"
        secondaryCta={t("careers.hero.secondary_cta")}
      ...
      <PageSection className="section-padding bg-brand-dark/10 backdrop-blur">
        <div className="container-wide">
          <div className="max-w-3xl mx-auto text-center mb-12">
            ...
            <ContentText
              contentKey="careers.openings.subtitle"
              value={t("careers.openings.subtitle")}
              as="p"
          ...
          {careers.length ? (
                  ...
                  <div className="p-6 sm:p-8">
                    <div className="flex flex-wrap items-center gap-2 mb-3">
                      <span className="inline-flex rounded-full bg-brand-sunlight px-3 py-1 text-xs font-bold uppercase tracking-widest text-brand-dark">
                        {career.is_expired ? t("careers.status.closed") : t("careers.status.open")}
                      </span>
                    </div>
                    <h3 className="font-heading text-2xl font-semibold text-white">{career.title}</h3>
                    <p className="mt-3 text-white/80">
                      {t("careers.deadline_prefix")} {formatDeadline(career.application_deadline)}
                    </p>
                    {!career.is_expired ? (
                      <Link
                        href="/contact"
            ...
            <ContentText
              contentKey="careers.empty"
              value={t("careers.empty")}
              as="div"
